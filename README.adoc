...
JUST STARTED. EXPERIMENTAL AT BEST.
...
This is licensed under the BSD 3-clause license. See link:LICENSE[].
...
* **"Instances":** again this does not refer to "instances" of OO classes,
  ...
  which means the default `procfs-scala` package instance definitions can be
...
* **Functions:** I have a trait named `Functions`, which along side the
  ...
  the `procfs-scala` library/toolkit.
